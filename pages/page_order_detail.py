...
def show_page():
    ...
    # 전역 CSS 스타일 적용 (모든 테이블 가운데 정렬)
    st.markdown("""
    <style>
    /* 모든 dataframe 요소를 가운데 정렬 - 강력한 선택자 사용 */
    ...
    /* Streamlit의 기본 스타일 덮어쓰기 */
    .row-widget.stDataFrame > div > div > div > div > table td,
    .row-widget.stDataFrame > div > div > div > div > table th {
        text-align: center !important;
    }
    </style>
    ...
    try:
        ...
        if details:
            ...
            for detail in details:
                detail_data.append({
                    ...
                    "주문수량": f"{detail.order_qty:,}개",
                    "단가": f"{detail.unit_price:,.0f}원",
                    "금액": f"{detail.order_qty * detail.unit_price:,.0f}원",
                    "출하수량": f"{detail.shipping_qty:,}개" if detail.shipping_qty else "0개",
                    "출하금액": f"{detail.shipping_amount:,.0f}원" if detail.shipping_amount else "0원",
                })
            
            detail_df = pd.DataFrame(detail_data)
            # 모든 내용 가운데 정렬 - 모든 컬럼에 명시적 적용
            styled_df = detail_df.style.set_properties(**{
                'text-align': 'center',
        ...
        if receipt_status:
            ...
            # 모든 내용 가운데 정렬 - 모든 컬럼에 명시적 적용
            styled_df = receipt_df.style.set_properties(**{
                'text-align': 'center',
                'vertical-align': 'middle'
            })
            styled_df = styled_df.set_table_styles([
        ...
        if shipping_plans:
            ...
            # 모든 내용 가운데 정렬 - 모든 컬럼에 명시적 적용
            styled_df = plan_df.style.set_properties(**{
                'text-align': 'center',
                'vertical-align': 'middle'
            })
            styled_df = styled_df.set_table_styles([
        ...
        if receipt_status:
            ...
            # 모든 내용 가운데 정렬 - 모든 컬럼에 명시적 적용
            styled_df = inventory_df.style.set_properties(**{
                'text-align': 'center',
                'vertical-align': 'middle'
            })
            styled_df = styled_df.set_table_styles([
